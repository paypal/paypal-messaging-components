import { getMutations } from '@renderer/locale';
...
jest.mock('@renderer/locale/US/PPC/mutations/niq', () => ({
    'layout:text': [
        [
            'default',
            {
...
jest.mock('@renderer/locale/US/PPC/mutations/ni', () => ({
    'layout:text': ['text', 'US', 'NI'],
    'layout:flex': ['flex', 'US', 'NI']
}));

jest.mock('@renderer/locale/US/PPC/mutations/ezp_any_eqz', () => ({
    'layout:text': ['text', 'US', 'EZP:ANY:EQZ'],
    'layout:flex': ['flex', 'US', 'EZP:ANY:EQZ']
}));

jest.mock('@renderer/locale/US/PPC/mutations/pala_single_eqz', () => ({
    'layout:text': ['text', 'US', 'PALA:SINGLE:EQZ'],
    'layout:flex': ['flex', 'US', 'PALA:SINGLE:EQZ']
}));

jest.mock('@renderer/locale/US/GPL/mutations/gplq', () => ({
    'layout:text': ['text', 'US', 'GPLQ'],
    'layout:flex': ['flex', 'US', 'GPLQ']
}));

jest.mock('@renderer/locale/DE/mutations/inst_any_eqz', () => ({
    'layout:text': ['text', 'DE', 'INST:ANY:EQZ'],
    'layout:flex': ['flex', 'DE', 'INST:ANY:EQZ']
}));

jest.mock('@renderer/locale/DE/mutations/palaq_any_eqz', () => ({
