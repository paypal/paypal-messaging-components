...
describe.each([
    ...
    test(
        `show error message when entering a non-qualifying amount inside calculator ${groupString}`,
        nonQualErrorEZP(account, viewport, bannerStyles)
    );
    test(
        `when amount is passed to EZP message, correct financing terms load in table ${groupString}`,
    ...
    test(
        `clicking EZP message loads EZP content with calculator for finance terms ${groupString}`,
