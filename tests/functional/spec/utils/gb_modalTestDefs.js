...
export const gbModalContent = (account, viewport, bannerStyle) => async () => {
    ...
    await modalFrame.waitForSelector(selectors.modal.headlineContainer);
    await modalFrame.waitForSelector(selectors.modal.h1);
    const h1 = await modalFrame.evaluate(() => document.querySelector('.headline-container h1').innerHTML);
