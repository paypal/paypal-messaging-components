...
describe('deviceID consistency on prop update', () => {
    const config = {
        account: 'DEV0000000GPL',
        amount: 100,
        style: bannerStyles[0]
    ...
    test('deviceID remains the same after prop update', async () => {
        const messageRequests = [];

        page.on('request', request => {
            const url = request.url();
            if (url.includes('/smart/message')) {
        ...
        await setupTestPage({ config, testPage: 'banner.html' });
        await page.waitFor(5 * 1000);

        await page.evaluate(() => {
            window.paypal.Messages({ amount: 200 }).render('.container');
        });

        await page.waitFor(5 * 1000);

        expect(messageRequests.length).toBeGreaterThanOrEqual(2);

        const initialDeviceID = messageRequests[0].deviceID;
        const updateDeviceID = messageRequests[1].deviceID;

        expect(initialDeviceID).toBeDefined();
        expect(updateDeviceID).toBeDefined();
        expect(updateDeviceID).toBe(initialDeviceID);
    });
});
