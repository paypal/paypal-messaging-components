...
const findSwitchCase = (source, country, reason) => {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (line.endsWith(needle)) {
            ...
            if (next && /^[ \t]*$/.test(caseIndent)) {
                const returnMatch = next.match(/^([ \t]*)return (.+);$/);
...
const registerLocale = spec => {
    ...
    const match = findSwitchCase(
        original,
        spec.templateCountry,
        `could not read the ${spec.templateCountry} switch case in ${LOCALE_INDEX} to mirror its signature`
    );
    ...
    if (!hasCase) {
        const anchor = findSwitchCase(
            updated,
            spec.templateCountry,
            `could not re-read the ${spec.templateCountry} switch case in ${LOCALE_INDEX} after updating imports`
        );
