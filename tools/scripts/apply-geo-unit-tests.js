...
const lastMockEnd = (source, country) => {
    ...
    while (from < source.length) {
        ...
        if (idx === -1) {
            ...
            const atLineStart = idx === 0 || source[idx - 1] === '\n';
            const close = atLineStart ? source.indexOf(closeToken, idx) : -1;
            if (atLineStart && close !== -1) {
...
const lastRowEnd = (source, country) => {
    ...
    while (from < source.length) {
        ...
        if (idx === -1) {
            ...
            const atLineStart = idx === 0 || source[idx - 1] === '\n';
            const close = atLineStart ? source.indexOf("']", idx + prefix.length) : -1;
            if (!atLineStart || close === -1) {
                from = idx + prefix.length;
            } else {
                const afterClose = close + 2;
                const comma = source[afterClose] === ',' ? 1 : 0;
                const newlineAt = afterClose + comma;
                if (source[newlineAt] === '\n') {
                    end = newlineAt + 1;
                    from = end;
                } else {
